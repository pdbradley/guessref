class GameRoundPreview < ViewComponent::Preview
  ...
  def queued_round
    game_round = FactoryBot.build_stubbed(:game_round, status: GameRound::QUEUED_STATUS)
  end
  ...
  def completed_round
    game_round = FactoryBot.build_stubbed(:game_round, status: GameRound::COMPLETED_STATUS)
  end
